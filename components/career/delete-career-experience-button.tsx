...
type DeleteCareerExperienceButtonProps = {
  companyName: string;
};

export function DeleteCareerExperienceButton({
  companyName,
}: DeleteCareerExperienceButtonProps) {
  const { pending } = useFormStatus();

  function handleClick(event: MouseEvent<HTMLButtonElement>) {
    const isConfirmed = window.confirm(
      `「${companyName}」の職務経歴を削除します。削除後は元に戻せません。よろしいですか？`,
    );
